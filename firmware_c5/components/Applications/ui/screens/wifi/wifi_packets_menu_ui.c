...
static lv_obj_t * screen_wifi_packets_menu = NULL;
...
static const wifi_packets_item_t wifi_packets_list[] = {
    {"SNIFFER RAW",       LV_SYMBOL_SHUFFLE, SCREEN_WIFI_SNIFFER_RAW},
    {"PMKID ATTACK",      LV_SYMBOL_WARNING, SCREEN_WIFI_SNIFFER_ATTACK},
    {"HANDSHAKE CAPTURE", LV_SYMBOL_WIFI,    SCREEN_WIFI_SNIFFER_HANDSHAKE}
};

#define WIFI_PACKETS_COUNT (sizeof(wifi_packets_list) / sizeof(wifi_packets_list[0]))
...
static void wifi_packets_item_event_cb(lv_event_t * e) {
    ...
    if(code == LV_EVENT_FOCUSED) {
        ...
        if(key == LV_KEY_ESC || key == LV_KEY_LEFT) {
            ...
            if(wifi_packets_list[index].target_screen == SCREEN_NONE) return;
            buzzer_play_sound_file("buzzer_hacker_confirm");
            ui_switch_screen(wifi_packets_list[index].target_screen);
        }
    }
...
void ui_wifi_packets_menu_open(void) {
    ...
    init_styles();
    if(screen_wifi_packets_menu) lv_obj_del(screen_wifi_packets_menu);

    screen_wifi_packets_menu = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(screen_wifi_packets_menu, current_theme.screen_base, 0);

    header_ui_create(screen_wifi_packets_menu);
    footer_ui_create(screen_wifi_packets_menu);

    lv_obj_t * menu = lv_obj_create(screen_wifi_packets_menu);
    ...
    for(int i = 0; i < WIFI_PACKETS_COUNT; i++) {
        ...
        lv_obj_add_event_cb(btn, wifi_packets_item_event_cb, LV_EVENT_ALL, label_sel);
    ...
    lv_screen_load(screen_wifi_packets_menu);
}
