...
static const wifi_attack_item_t wifi_attack_list[] = {
    {"DEAUTH ATTACK", LV_SYMBOL_WARNING, SCREEN_WIFI_DEAUTH_ATTACK},
    {"BEACON SPAM",   LV_SYMBOL_WIFI,    SCREEN_WIFI_BEACON_SPAM_SIMPLE},
    {"PROBE FLOOD",   LV_SYMBOL_SHUFFLE, SCREEN_WIFI_PROBE_FLOOD},
    {"AUTH FLOOD",    LV_SYMBOL_CLOSE,   SCREEN_WIFI_AUTH_FLOOD}
};

#define WIFI_ATTACK_COUNT (sizeof(wifi_attack_list) / sizeof(wifi_attack_list[0]))
...
static void wifi_attack_item_event_cb(lv_event_t * e) {
    ...
    if(code == LV_EVENT_FOCUSED) {
        ...
        if(key == LV_KEY_ESC || key == LV_KEY_LEFT) {
            ...
            if(wifi_attack_list[index].target_screen == SCREEN_NONE) return;
            buzzer_play_sound_file("buzzer_hacker_confirm");
            ui_switch_screen(wifi_attack_list[index].target_screen);
        }
    }
...
void ui_wifi_attack_menu_open(void) {
    ...
    init_styles();
    if(screen_wifi_attack_menu) lv_obj_del(screen_wifi_attack_menu);

    screen_wifi_attack_menu = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(screen_wifi_attack_menu, current_theme.screen_base, 0);

    header_ui_create(screen_wifi_attack_menu);
    footer_ui_create(screen_wifi_attack_menu);

    lv_obj_t * menu = lv_obj_create(screen_wifi_attack_menu);
    ...
    for(int i = 0; i < WIFI_ATTACK_COUNT; i++) {
        ...
        lv_obj_add_event_cb(btn, wifi_attack_item_event_cb, LV_EVENT_ALL, label_sel);
