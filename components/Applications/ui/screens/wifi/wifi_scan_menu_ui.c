...
static const wifi_scan_item_t wifi_scan_list[] = {
    {"SCAN APS",      LV_SYMBOL_WIFI, SCREEN_WIFI_SCAN_AP},
    {"SCAN STATIONS", LV_SYMBOL_WIFI, SCREEN_WIFI_SCAN_STATIONS},
    {"SCAN TARGETED", LV_SYMBOL_WIFI, SCREEN_WIFI_SCAN_TARGET},
    {"PROBE SNIFFER", LV_SYMBOL_WIFI, SCREEN_WIFI_SCAN_PROBE},
    {"PACKET MONITOR",LV_SYMBOL_WIFI, SCREEN_WIFI_SCAN_MONITOR}
};

#define WIFI_SCAN_COUNT (sizeof(wifi_scan_list) / sizeof(wifi_scan_list[0]))
...
static void wifi_scan_item_event_cb(lv_event_t * e) {
    ...
    if(code == LV_EVENT_FOCUSED) {
        ...
        if(key == LV_KEY_ESC || key == LV_KEY_LEFT) {
            ...
            if(wifi_scan_list[index].target_screen == SCREEN_NONE) return;
            buzzer_play_sound_file("buzzer_hacker_confirm");
            ui_switch_screen(wifi_scan_list[index].target_screen);
        }
    }
...
void ui_wifi_scan_menu_open(void) {
    ...
    init_styles();
    if(screen_wifi_scan_menu) lv_obj_del(screen_wifi_scan_menu);

    screen_wifi_scan_menu = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(screen_wifi_scan_menu, current_theme.screen_base, 0);

    header_ui_create(screen_wifi_scan_menu);
    footer_ui_create(screen_wifi_scan_menu);

    lv_obj_t * menu = lv_obj_create(screen_wifi_scan_menu);
    ...
    for(int i = 0; i < WIFI_SCAN_COUNT; i++) {
        ...
        lv_obj_add_event_cb(btn, wifi_scan_item_event_cb, LV_EVENT_ALL, label_sel);
